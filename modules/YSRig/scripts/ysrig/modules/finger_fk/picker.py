        ...
        for side, grp in zip(side_list, grp_list):
            ...
            for names in names_chunk:
                if len(names) == 1:
                    ...
                    finger_name = names[-1].replace("_GB", "")
                    buttons = []
                    for fng_name, bt_name in zip(names, FINGER_BTN_NAMES):
                        if bt_name == "Roll":
                            fng_name = fng_name.replace("_GB", "_All")
            ...
            datas += [gui.PickerModuleData(name=grp, position={'x': 0, 'y': 0}, buttons=button, side=side, mirror=mirror)]
        self.datas = datas