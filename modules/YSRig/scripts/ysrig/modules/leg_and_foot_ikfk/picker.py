...
def ik_fk_matching(meta_node, side, switch):
    ...
    if ikfk_current_value:
        ...
        if len(ik_joints) == 5:
            ...
            cmds.setKeyframe(f"{rev_fk_ctrl[1]}.rotate")

        cmds.setKeyframe(f"{ik_ctrl}.translate")
        cmds.setKeyframe(f"{pv_ctrl}.translate")
        cmds.setKeyframe(f"{rev_fk_ctrl[0]}.rotateZ")
        ...
        if len(ik_joints) == 5:
            ...
            cmds.setKeyframe(f"{fk_ctrls[4]}.rotate")

        cmds.setKeyframe(f"{fk_ctrls[0]}.rotate")
        cmds.setKeyframe(f"{fk_ctrls[1]}.rotateZ")
        cmds.setKeyframe(f"{fk_ctrls[2]}.rotate")
        cmds.setKeyframe(f"{fk_ctrls[3]}.rotateZ")
